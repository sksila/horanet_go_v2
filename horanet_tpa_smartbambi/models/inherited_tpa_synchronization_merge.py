...
class TPASynchronizationMergeSmartBambi(models.Model):
    ...
    # region Actions
    @api.model
    def cron_action_merge_smartbambi(self, options):
        """Cron method to merge SmartBambi TPA periodically.
        ...
        :return: nothing
        """
        _logger.info("Cron method cron_action_merge_smartbambi called")
        if self.env['collectivity.config.settings'].get_tpa_smartbambi_is_enable():
            ...
            for rec_to_merge in recs_to_merge:
                if rec_to_merge:
                    ...
                    rec_to_merge.smartbambi_merge()

    # endregion

    # region Model methods
    @api.multi
    def smartbambi_merge(self, context={}, **kwargs):
        """Entry point of TPA SmartBambi synchronization merge service.

        :return: nothing
        """
        try:
            force = context.get('force', False)
            if force or self.env['collectivity.config.settings'].get_tpa_smartbambi_is_enable():
                data = {}
                data['external_id_src'] = str(self.external_id_src)
                data['external_id_dest'] = str(self.external_id_dest)

                partner_merge = self
                url = partner_merge._smartbambi_getSetting_url(self)
                data_merge = []
