...
class ResPartner(models.Model):
    """Override partner model to add Aquagliss TPA synchronization membership."""
    ...
    tpa_membership_aquagliss = fields.Boolean(string="Synchronize with Aquagliss")
    ...
    def write(self, vals):
        ...
        if len(set(vals.keys()) - fields_not_tpa):
            for rec in self:
                if rec.tpa_membership_aquagliss:
                    ...
                    if self.env['collectivity.config.settings'].get_tpa_aquagliss_is_enable():
                        ...
                        tpa_synchronization_status_rec.execute_method_in_thread('action_synchronization_aquagliss')
        return res
    ...
    def create(self, vals):
        ...
        if company_type_not_foyer and self.env.user.has_group('horanet_tpa_aquagliss.group_tpa_aquagliss'):
            vals.update({'tpa_membership_aquagliss': True})

        new_partner = super(ResPartner, self).create(vals)
        if new_partner.tpa_membership_aquagliss:
