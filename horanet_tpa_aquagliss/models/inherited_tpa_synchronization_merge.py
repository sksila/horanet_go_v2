...
class TPASynchronizationMergeAquagliss(models.Model):
    ...
    # region Actions
    @api.model
    def cron_action_merge_aquagliss(self, options):
        """Cron method to merge Aquagliss TPA periodically.
        ...
        :return: nothing
        """
        _logger.info("Cron method cron_action_merge_aquagliss called")
        if self.env['collectivity.config.settings'].get_tpa_aquagliss_is_enable():
            ...
            for rec_to_merge in recs_to_merge:
                if rec_to_merge:
                    ...
                    rec_to_merge.aquagliss_merge()

    # # endregion

    # region Model methods
    @api.multi
    def aquagliss_merge(self, context={}, **kwargs):
        """Entry point of TPA Aquagliss synchronization merge service.
        ...
        write_result = None
        try:
            if force or self.env['collectivity.config.settings'].get_tpa_aquagliss_is_enable():
                data = {}
                data['external_id_src'] = str(self.external_id_src)
                data['external_id_dest'] = str(self.external_id_dest)

                partner_merge = self
                url = partner_merge._aquagliss_getSetting_url(self)
                data_merge = []
