...
class SynchroTPAAquagliss(models.Model):
    ...
    tpa_name = fields.Selection(selection_add=[(TPA_NAME, "Aquagliss")])
    ...
    # region Actions
    @api.model
    def cron_action_synchronization_aquagliss(self, options):
        """Cron method to synchronize Aquagliss TPA periodically.
        ...
        :return: nothing
        """
        _logger.info("Cron method cron_action_synchronization_aquagliss called")
        if self.env['collectivity.config.settings'].get_tpa_aquagliss_is_enable():
            search_filter = []
            ...
            search_filter.append(('try_number', '=', False))
            ...
            if rec_to_update:
                ...
                rec_to_update.aquagliss_synchronization()

    @api.multi
    def action_synchronization_aquagliss(self, context={}, **kwargs):
        ...
        :return: nothing
        """
        tpa_partners_to_sync = self.filtered(lambda x: x.ref_partner.tpa_membership_aquagliss)
        force = context.get('force', False)
        # Check if partner synchronization is needed
        if (force or self.env['collectivity.config.settings'].get_tpa_aquagliss_is_enable()) and \
                len(tpa_partners_to_sync):
            tpa_partners_to_sync.aquagliss_synchronization()
    ...
    @api.multi
    def aquagliss_synchronization(self):
        """Entry point of TPA Aquagliss synchronization service.
        ...
        try:
            ...
            data_partners = self.aquagliss_get_data()
    ...
    def aquagliss_get_data(self):
        """Build data partner list for TPA Aquagliss.

        :return: data partner list
        """
        partners_data = []
        for rec in self:
            partners_data.append(SynchroTPAAquagliss._aquagliss_get_data_partner(rec.ref_partner))
    ...
    @staticmethod
    def _aquagliss_get_data_partner(partner, recursion=True):
        """Build dictionary of data partner sent to TPA Aquagliss.

        :param partner: Record of res.partner
        :param recursion: If False, no propagation (avoid cyclic recursion)
        :return: partner data formatted for Aquagliss system
        """
        data = {}
        ...
        if partner.company_type == 'residence':
            ...
            data = SynchroTPAAquagliss._data_foyer(data, partner)
        else:
            ...
            if recursion:
                data['relation_foyers'] = [
                    {'date_debut': relation.begin_date,
                     ...
                     'foyer': SynchroTPAAquagliss._aquagliss_get_data_partner(relation.foyer_id, recursion=False)
                     }
                    for relation in p.foyer_relation_ids]
                data['dependants'] = [SynchroTPAAquagliss._aquagliss_get_data_partner(partner_dep, recursion=False)
                                      for partner_dep in p.dependant_ids]
                data['garants'] = [SynchroTPAAquagliss._aquagliss_get_data_partner(partner_gar, recursion=False)
            ...
            for category in p.category_id:
                if data['profile_object']["name"] != 'ADULT' and data['profile_object']["name"] != 'CHILD':
                    if category.name == 'ADULTE':
                        ...
                        data['profile_object'] = {'id': '99', 'name': category.id and category.name}
                    else:
                        data['profile_object'] = {'id': '0', 'name': 'undefined'}
    ...
    def _aquagliss_getSetting_url(self):
        ...
        method_name = self.env['collectivity.config.settings'].get_tpa_aquagliss_partner_method()
