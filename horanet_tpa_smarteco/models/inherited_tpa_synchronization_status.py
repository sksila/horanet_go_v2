...
class SynchroTPASmarteco(models.Model):
    ...
    # region Actions
    @api.model
    def cron_action_synchronization_smarteco(self, options):
        """Cron method to synchronize SmartEco TPA periodically.
        ...
        :return: nothing
        """
        _logger.info("Cron method cron_action_synchronization_smarteco called")
        ...
        if self.env['collectivity.config.settings'].get_tpa_smarteco_is_enable():
            ...
            if rec_to_update:
                ...
                rec_to_update.smarteco_synchronization()

    @api.multi
    def action_synchronization_smarteco(self, context={}, **kwargs):
        ...
        :return: nothing
        """
        tpa_partners_to_sync = self.filtered(lambda x: x.ref_partner.tpa_membership_smarteco)
        force = context.get('force', False)
        # Check if partner synchronization is needed
        if (force or self.env['collectivity.config.settings'].get_tpa_smarteco_is_enable()) and \
                len(tpa_partners_to_sync):
            tpa_partners_to_sync.smarteco_synchronization()
    ...
    @api.multi
    def smarteco_synchronization(self):
        """Entry point of TPA SmartEco synchronization service.

        :return: nothing
        """
        try:
            # If partners to synchronize
            url = self._smarteco_getSetting_url()
            data_partners = self.smarteco_get_data()
            ...
            write_result = self.smarteco_get_request_write_result(request)
    ...
    def smarteco_get_request_write_result(self, request):
    ...
    def smarteco_get_data(self):
        """Build data partner list for TPA SmartEco.

        :return: data partner list
        """
        partners_data = []
        for rec in self:
            partners_data.append(SynchroTPASmarteco._smarteco_get_data_partner(rec.ref_partner))
    ...
    @staticmethod
    def _smarteco_get_data_partner(partner, recursion=True):
        """Build dictionary of data partner sent to TPA SmartEco.

        :param partner: Record of res.partner
        :param recursion: If False, no propagation (avoid cyclic recursion)
        :return: partner data formatted for SmartEco system
        """
        data = {}
        ...
        if partner.company_type == 'residence':
            ...
            data = SynchroTPASmarteco.data_foyer(data, partner)
        else:
            ...
            if recursion:
                data['relation_foyers'] = [
                    {'date_debut': relation.begin_date,
                     ...
                     'foyer': SynchroTPASmarteco._smarteco_get_data_partner(relation.foyer_id, recursion=False)
                     }
                    for relation in p.foyer_relation_ids]
                data['dependants'] = [SynchroTPASmarteco._smarteco_get_data_partner(partner_dep, recursion=False)
                                      for partner_dep in p.dependant_ids]
                data['garants'] = [SynchroTPASmarteco._smarteco_get_data_partner(partner_gar, recursion=False)
    ...
    @staticmethod
    def data_foyer(data, partner):
        data['name'] = partner.foyer_name
        foyer_members = partner.env['horanet.relation.foyer'].search_read([('id', 'in', partner.foyer_member_ids.ids)])
        for foyer_member in foyer_members:
            # Add responsible(s) into interface cluster
            if (foyer_member['is_responsible']):
                responsable_partner = partner.env['res.partner'].search_read([('id', '=',
                                                                               foyer_member["partner_id"][0])])
                ...
                # 'category': r['category_id'] and str(r['category_id'][1]),
    ...
    def _smarteco_getSetting_url(self):
        """Private method to get TPA SmartEco settings URL.

        :return: URL of TPA SmartEco web service
        """
        backend_url = self.env['collectivity.config.settings'].get_tpa_smarteco_backend_url()
        method_name = self.env['collectivity.config.settings'].get_tpa_smarteco_partner_method()
        if not method_name or not backend_url:
            raise exceptions.MissingError('TPA smartEco settings invalid (check url and method name)')
